...
describe("CheckStockAvailabilityUseCase", () => {
  let checkStockUseCase: CheckStockAvailabilityUseCase;

  beforeEach(() => {
    jest.clearAllMocks();
    checkStockUseCase = new CheckStockAvailabilityUseCase();
  });
  ...
  it("deve lançar erro se peça não for encontrada", async () => {
    ...
    await expect(checkStockUseCase.execute(999, 5)).rejects.toThrow(
      "Peça com ID 999 não encontrada"
    );
  });
});