...
describe("GetStockMovementsUseCase", () => {
    let getStockMovementsUseCase;
    beforeEach(() => {
        jest.clearAllMocks();
        getStockMovementsUseCase = new GetStockMovementsUseCase_1.GetStockMovementsUseCase();
    });
    it("deve retornar todas as movimentações sem filtros", async () => {
        ...
        StockMovementRepository_1.StockMovementRepository.createQueryBuilder.mockReturnValue(mockQueryBuilder);
    ...
    it("deve filtrar por partId", async () => {
        ...
        expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("movement.partId = :partId", { partId: 1 });
    });
    it("deve filtrar por serviceOrderId", async () => {
        ...
        expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("movement.serviceOrderId = :serviceOrderId", { serviceOrderId: 10 });
    });
    it("deve filtrar por período", async () => {
        ...
        expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("movement.createdAt >= :startDate", { startDate });
    ...
    it("deve filtrar por tipo", async () => {
        ...
        expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith("movement.type = :type", { type: "IN" });
    });
