...
async function sendStatusEmail(to, orderId, status) {
    try {
...
async function sendBudgetEmail(to, orderId, budget, items) {
    try {
        if (!transporter) {
            initEmailService();
        }
        const itemsList = items.map(item => `- ${item.title}: R$ ${item.total.toFixed(2)}`).join('\n');
        const mailOptions = {
            from: `"Oficina Mecânica" <${process.env.SMTP_FROM || 'no-reply@oficina.com'}>`,
            to,
